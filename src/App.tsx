...
// Locally: copy .env.example ΓåÆ .env and fill in your values.
// On GitHub: add each value as a Repository Secret (Settings ΓåÆ Secrets ΓåÆ Actions).
...
// Public fallback URLs ΓÇö shown before auth or if Storage fetch fails
...
const REVIEWS = [
  ...
  },
  {
    name: "Belmin Husic",
    rating: 5,
    text: "I've been going to Ashor's Fade for the past few weeks, and every single cut has been fire. The barbers truly know what they're doing ΓÇö clean fades, perfect shape-ups, and they take the time to get every detail right.",
    date: "4 months ago"
  },
  {
    name: "Ben Dover",
...
// Gallery titles ΓÇö URLs are resolved from Firebase Storage (step 5)
...
function BarberApp() {
  ...
  // ΓöÇΓöÇ Secure image URLs resolved from Firebase Storage ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.bookings && Array.isArray(data.bookings)) {
            ...
            console.log('Γ£à Synced', sheetBookings.length, 'bookings from Google Sheet');
          }
        }
        setIsAvailabilityLoaded(true);
      } catch (err) {
        console.warn('ΓÜá∩╕Å Could not fetch from Google Sheet, using local bookings:', err);
        setIsAvailabilityLoaded(true);
      }
    };
...
≡ƒôà Date: ${booking.date}
ΓÅ░ Time: ${booking.timeSlot}
Γ£é∩╕Å Service: ${booking.service}
≡ƒÆê Barber: ${booking.barber}
≡ƒô₧ Phone: ${booking.phone}
...
Ashor's Fade Barbershop`;
  ...
  const emailReminder = (booking: any) => {
    ...
    // Provide feedback
    setTimeout(() => {
      alert('Γ£à Email reminder sent! Check your email for booking details.');
    }, 300);
  };
  ...
  const handleBookingSubmit = async (e: React.FormEvent) => {
    ...
    if (hasConflict) {
      ...
      alert('Γ¥î This time slot is already booked! Please choose another date, time, or barber.');
      return;
    }
              ...
              <div className="max-w-2xl mx-auto px-6">
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl p-8 mb-12 space-y-4">
                  <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <div>
                      <p className="text-white/40 text-sm uppercase tracking-widest mb-1">≡ƒôà Date</p>
                      <p className="text-xl font-bold">{bookingForm.date}</p>
                    </div>
                    <div>
                      <p className="text-white/40 text-sm uppercase tracking-widest mb-1">ΓÅ░ Time</p>
                ...
                <div className="bg-white/5 border border-white/10 rounded-xl p-6 mb-12 text-sm">
                  <p className="text-white/60 leading-relaxed">
                    ≡ƒÆí <strong>Pro Tip:</strong> After downloading, you can set a 60-minute reminder in your calendar app. We'll also display your bookings below for easy access.
                  </p>
                </div>
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans selection:bg-white selection:text-black">
      ...
      {/* Hero ΓÇö STEP 5: secureImages.hero */}
      ...
      {/* About ΓÇö STEP 5: secureImages.about */}
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-20 items-center">
          ...
          <div>
            ...
            <div className="space-y-6 text-white/70 font-light leading-relaxed text-lg">
              ...
              <p>
                After 15 years, Ashor's Fade has become a place people trust. You don't just leave with a haircutΓÇöyou leave feeling confident and happy.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Gallery ΓÇö STEP 5: secureImages.gallery */}
